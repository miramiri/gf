...
# --- تغییر فونت (پذیرش فاصله و اعداد فارسی/انگلیسی)
@client.on(events.NewMessage(pattern=r"^\.ساعت\s+(\d+)$"))
async def set_font(event):
    txt_num = event.pattern_match.group(1)
    try:
        num = int(txt_num)  # اعداد فارسی هم کار می‌کند
    except ValueError:
        return await event.edit("❌ لطفاً عدد صحیح بده.")

    if num not in FONTS:
        return await event.edit("❌ فونت نامعتبره. از 1 تا 50 انتخاب کن. (.لیست ساعت)")

    # فقط این قسمت ممکن است در بعضی پیاده‌سازی‌ها خطا بدهد
    try:
        state["clock_font"] = num
        if callable(save_state):
            save_state()
    except Exception as e:
        # خطای واقعی را نشان بده تا معلوم شود مشکل از کجاست
        return await event.edit(f"⚠️ فونت ذخیره نشد: {e}")
